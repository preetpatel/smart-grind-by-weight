...
**Web App (`tools/web-server`):** One Next.js app (strict TypeScript, Biome, pnpm; scaffolded with `create-next-app@canary`) serves everything web: the flasher UI, the analytics dashboard, user accounts, the cloud sync API, and a firmware release proxy. Main routes: **/** (My Grinder — device panels: Get Started/USB install via esp-web-tools, Update/OTA, WiFi & Sync, Diagnostics — new visitors land on Get Started, returning ones on Update via a `grinderSeen` localStorage flag), **/analytics**, plus **/signin** and **/account** (Better Auth; masthead account slot in `components/account-menu.tsx` — see the Cloud Sync section for the auth model). Above the tabs sits a one-line device strip (`components/device-strip.tsx`) — a pairing prompt for new visitors, cached device snapshot (version, sessions on device, WiFi + backup state, update-available chip) for known ones; red is reserved for flash CTAs, the strip's Connect is accent blue. All BLE flows share one GATT connection via the `lib/client/ble.ts` singleton (chooser once, 30 s idle auto-release, clock sync + snapshot reads on every connect, silent background refresh on Chrome 117+ via `getDevices()`; known grinders persist in `grinderRegistry` localStorage keyed by device id; React consumes it through `useSyncExternalStore` in `lib/client/use-grinder.ts`). Operations that outlast the idle window — firmware upload, grind-data export, the diagnostics stream — must claim the link with the nesting `hold()`/`releaseHold()` lease (always paired in a `finally`); while any holder is active `release()` is a no-op, so a concurrent flow (typically the device strip's background snapshot refresh) can't arm the idle timer and disconnect mid-transfer. The **/analytics** page pulls grind data + a device health snapshot over Web Bluetooth (or syncs from the cloud store) and renders the full analysis dashboard (single-session, session-overlay compare, multi-session, long-term trends with a burr-wear odometer from lifetime stats, device health; Plotly via `plotly.js-dist-min`, pure figure builders in `lib/analytics/figures.ts`). The binary parser (`lib/parser.ts`) is shared by the browser dashboard AND the server's ingest validation — one TS parser; any `grind_logging.h` struct change must update it alongside the Python parser (see `tools/ble/CLAUDE.md`). Grind data chunks are delivered as acknowledged BLE indications; the firmware retries unconfirmed chunks rather than dropping them. Firmware releases are listed and streamed same-origin through `/api/firmware[/tag/asset]` (GitHub releases API + asset proxy) — there is no GitHub Pages deployment and no CI asset copying; the app deploys to Vercel or self-hosts via `docker compose up`.

**Cloud Sync (docs/CLOUD_SYNC.md):** Grind sessions upload to a cloud store — hosted (Vercel) or self-hosted (`tools/web-server`, `docker compose up`: Next.js app + Postgres via Drizzle). The device holds `{server_url, store_id, upload_key, view_key}` in the `cloudsync` NVS namespace, provisioned over BLE from the web app's **WiFi & Sync** panel (`BLE_SYSINFO_CLOUD_CONFIG_CHAR_UUID`, write-only, same deferred-write pattern as WiFi; JSON readback on `BLE_SYSINFO_CLOUD_STATUS_CHAR_UUID` deliberately hands out `store_id`+`view_key` — read-only claim-by-possession — but never the upload key). Off until provisioned. `CloudSync` (`src/system/cloud_sync.*`) is the second `WifiService` consumer: after SNTP the still-open window enters an `UPLOADING` state that runs one blocking HTTP op per loop pass (grind/OTA/export gating re-checked between ops) — manifest handshake (one `(session_id, timestamp, size, checksum)` tuple per file on flash; server replies with what it lacks), one verbatim-bytes POST per wanted session, then a compact health snapshot. Zero sync state on the device; a wiped server just asks for everything again. New sessions bring a window forward after a 15 s settle. HTTPS validates via the ESP-IDF CA bundle; plain http works for LAN self-hosts. Session headers now carry a real zlib CRC-32 (legacy files have 0; the server verifies when nonzero). Server-side: raw blob is truth (Postgres `bytea`), an 80-byte summary parsed at ingest is the query index, dedup on `(store_id, sha256)` — so a factory-reset device's reborn session ids coexist and re-uploads are dropped silently. **Stores belong to user accounts** (Better Auth in `tools/web-server/lib/auth-server.ts`: email/password always, passkeys on top, GitHub button when `GITHUB_CLIENT_ID/SECRET` are set; `BETTER_AUTH_SECRET` required in production, no password reset by design — no mail service). Browser store management (create/rename/delete/provision/rotate) is owner-session-only; `authStore` tries the owner's cookie before bearer keys, so the device's key-authed ingest is untouched. Two keys, one job each: `upload_key` is the device credential, stored hashed and **rotated on every provision** (`POST /api/stores/[id]/provision` — any signed-in owner browser can provision; a DB dump leaks no write credential); `view_key` is the semi-public read credential (stored plaintext) behind share links and BLE claims. Per-account store cap (`SYNC_STORES_PER_USER`) replaced the per-IP limit and provisional-store GC; 10k-session quota rotates oldest (off in self-host). The Analytics tab resolves its cloud source owned-stores-first (picker when an account owns several) then viewer link (records keyed by content hash in IndexedDB v2; BLE pulls retain raw bytes, auto-backfill into owned stores through the same idempotent ingest via the session cookie, and read-only dashboard links share via `#store=id:view_key`). Server ingest and the dashboard share `tools/web-server/lib/parser.ts` — one TS parser; the Python parser remains a separate consumer of `grind_logging.h`. Menu → Settings → Cloud Sync mirrors the WiFi page (toggle, status rows, Forget Sync). The server app is strict TypeScript linted by Biome; checks in `tools/web-server`: `pnpm test` (vitest + PGlite), `pnpm typecheck`, `pnpm lint`.
