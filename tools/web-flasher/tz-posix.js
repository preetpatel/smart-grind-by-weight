...
function posixOffset(minutesEast) {
...
function findTransition(before, after, timeZone) {
...
function posixRuleFor(transition, offsetBeforeMin, timeZone) {
...
// Plain script (not a module): consumed by flasher.js via window.detectPosixTz.
function detectPosixTz() {
    ...
    samples.push({
        date: new Date(Date.UTC(year + 1, 0, 1, 12, 0, 0)),
        offset: offsetMinutesAt(new Date(Date.UTC(year + 1, 0, 1, 12, 0, 0)), zoneName),
    });

    const offsets = [...new Set(samples.map(s => s.offset))];
    if (offsets.length === 1) {
        return { rule: `STD${posixOffset(offsets[0])}`, zoneName };
    }
    ...
    for (let i = 1; i < samples.length; i++) {
        if (samples[i].offset !== samples[i - 1].offset) {
            transitions.push({
                at: findTransition(samples[i - 1].date, samples[i].date, zoneName),
                from: samples[i - 1].offset,
                to: samples[i].offset,
            });
        }
    }
    ...
    const startRule = posixRuleFor(toDst.at, toDst.from, zoneName);
    const endRule = posixRuleFor(toStd.at, toStd.from, zoneName);
