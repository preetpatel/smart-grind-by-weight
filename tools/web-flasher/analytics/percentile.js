...
function simpleFlowRate(samples, uptoIndex, currentTs, windowMs) {
...
function percentile95At(samples, uptoIndex, currentTs, windowMs, subWindowMs, stepMs) {
    if (uptoIndex + 1 < MIN_SAMPLES_FOR_PERCENTILE) {
        return simpleFlowRate(samples, uptoIndex, currentTs, windowMs);
    }

    const minWindowForSamples = Math.floor((MIN_SAMPLES_FOR_PERCENTILE * 1000) / HW_LOADCELL_SAMPLE_RATE_SPS);
...
export function percentile95Series(samples, { windowMs = 200, subWindowMs = 300, stepMs = 100 } = {}) {
