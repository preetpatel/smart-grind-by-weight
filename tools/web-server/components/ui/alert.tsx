    ...
    "group/alert relative grid w-full gap-0.5 rounded-2xl border px-4 py-3 text-left text-sm has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2.5 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-4",
    {
        variants: {
            variant: {
                ...
                // Added for this app: status regions grade four ways (device flows,
                // diagnostics, cloud sync), which the stock two variants can't say.
                success:
                    'bg-card text-success *:data-[slot=alert-description]:text-success/90 *:[svg]:text-current',
                caution:
                    'bg-card text-caution *:data-[slot=alert-description]:text-caution/90 *:[svg]:text-current',
            },
        },
        defaultVariants: {
            variant: 'default',
