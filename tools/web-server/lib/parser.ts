// Binary parser for grind session files, used by BOTH the server ingest
// (app/api/stores/[id]/sessions) and the browser dashboard — one TypeScript
// parser for every JS consumer.
//
// The layout must stay aligned with the packed structs in
// src/logging/grind_logging.h and the Python parser in tools/ble/grinder-ble.py
// (_parse_single_file_data). See tools/ble/CLAUDE.md: every struct change on
// the firmware side requires updating this file too.
//
...
export const PROFILE_MAP: Record<number, string> = { 0: 'SINGLE', 1: 'DOUBLE', 2: 'CUSTOM' };
export const MODE_MAP: Record<number, string> = { 0: 'WEIGHT', 1: 'TIME' };
...
// Parses one session file. Throws on any validation failure, mirroring the
// Python parser's behaviour of rejecting the whole session rather than
...
export function parseSessionFile(
    ...
    if (schemaVersion !== LOG_SCHEMA_VERSION) {
        warnings.push(
            `Session ${expectedSessionId} uses schema ${schemaVersion}, expected ${LOG_SCHEMA_VERSION}. Attempting to parse anyway.`,
        );
    }
    ...
    const resultStatus = new TextDecoder().decode(
        nul === -1 ? resultBytes : resultBytes.subarray(0, nul),
    );

    const session: ParsedGrindSession = {
    ...
    for (let i = 0; i < eventCount; i++) {
        ...
        if (eventSequenceId !== expectedEventSequence) {
            throw new Error(
                `Event sequence out of order: expected ${expectedEventSequence}, got ${eventSequenceId} at event ${i}`,
            );
        }
    ...
    const measurements: ParsedGrindMeasurement[] = [];
    ...
    for (let i = 0; i < measurementCount; i++) {
        ...
        if (sequenceId !== expectedMeasurementSequence) {
            throw new Error(
                `Session ${sessionId} corrupted: measurement sequence error at index ${i} (expected ${expectedMeasurementSequence}, got ${sequenceId})`,
            );
        }
